...
#   filename:  new_yamls/app_9.yaml
#   timestamp: 2022-10-11T21:09:46+00:00
...
class CardInfoResponse(BaseModel):
    maskedPan: constr(max_length=50) = Field(..., description="Маскированый PAN", example="489350******8081")
    embossingName: constr(max_length=50) = Field(..., description="Эмбоссированное имя клиента", example="ИМЯ ФАМИЛИЯ")
    cardExpiry: constr(regex=r"^(0[1-9]|1[0-2])/?([0-9]{2})$", max_length=5) = Field(
        ..., description="Дата окончания действия карты", example="MM/yy"
    )
    publicId: constr(max_length=50) = Field(..., description="Публичный идентификатор продукта")
...
class CardOpenRequest(BaseModel):
    partnerId: constr(max_length=50) = Field(..., description="Идентификатор партнера")
