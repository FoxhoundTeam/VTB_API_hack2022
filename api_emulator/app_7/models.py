...
#   filename:  new_yamls/app_5.yaml
#   timestamp: 2022-10-11T21:09:47+00:00
...
class CardInfoResponse(BaseModel):
    correlationId: Optional[constr(max_length=500)] = Field(
        None, description="Идентификатор сообщения, на которое дается ответ"
    )
    errorCode: constr(max_length=500) = Field(..., description="Код ошибки")
    errorText: Optional[constr(max_length=5000)] = Field(None, description="Текст ошибки")
    requestTime: Optional[datetime] = Field(None, description="Дата и время формирования сообщения")
...
class CardStatusRequest(BaseModel):
    newStatus: NewStatus = Field(..., description="Новый статус карты", example="ACTIVE")
    reason: Optional[constr(max_length=500)] = Field(None, description="Причина блокировки/разблокировки карты")


class PinCardRequest(BaseModel):
    Pin: constr(max_length=500) = Field(..., description="Шифрованный ПИН-код.")
    publicKeyId: constr(max_length=500) = Field(
        ..., description="Идентификатор публичного ключа, выданный криптосервером."
    )
